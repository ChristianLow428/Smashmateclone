...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!playerEmail) {
      return NextResponse.json({ error: 'playerEmail parameter required' }, { status: 400 })
    }
    
    console.log('Testing rating system for player:', playerEmail)
    
    // Get current rating
    const { data: currentRating, error: ratingError } = await supabase
      .from('player_ratings')
      .select('*')
      .eq('player_id', playerEmail)
      .single()
    ...
    return NextResponse.json({
      ...
      message: 'Rating system test completed'
    })
    
  } catch (error) {
    console.error('Error in rating system test:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Manual rating test:', { player1Email, player2Email, winner })
    ...
    console.error('Error in manual rating test:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
